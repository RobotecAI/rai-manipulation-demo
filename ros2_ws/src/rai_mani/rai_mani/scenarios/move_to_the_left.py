...
class MoveToTheLeft(ScenarioBase):
    ...
    def calculate_progress(self):
        ...
        num_good_apples = sum(1 for name in self.entities if name.startswith('apple') and self.pose_transformed(self.get_entity_pose(name)).position.y < 0.0)
            ...
            if len(self.manipulator_queue) == 0:
                for name in self.entities:
                    ...
                    if pose.position.y > 0.0:
                        self.node.get_logger().info(f'Moving {name} to the left from {pose.position.y}')
