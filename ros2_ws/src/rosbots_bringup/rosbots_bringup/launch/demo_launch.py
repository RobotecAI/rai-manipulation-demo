...
def sequential_launch(cmds):
    """
    Sequentially launch the given commands.
    """
    actions = []
    for i in range(len(cmds)):
        prev_action = cmds[i - 1] if i > 0 else None
        if prev_action:
            actions.append(
                RegisterEventHandler(
